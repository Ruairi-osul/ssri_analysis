...
df_sc %>%
  ...
  ggsave(file.path(fig_dir, "SRSR_RscDist.svg"))
...
df_cc %>%
  ...
  geom_histogram(bins=80, alpha=0.7, fill="#332288") +
  geom_vline(xintercept=0, linetype="dashed", lwd=1) +
  ...
  ggsave(file.path(fig_dir, "SRSR_CCLDist.svg"))
...
df_cc %>%
  ...
  ggsave(file.path(fig_dir, "SRSR_CCSDist.svg"))
...
df_ev %>%
  ...
  ggsave(file.path(fig_dir, "SRSR_EvDist.svg"))
...
df_sc %>%
  ...
  geom_col(color="black", lwd=2, fill="#332288", alpha=0.7) +
  ...
  ggsave(file.path(fig_dir, "SRSR_RscPercent.svg"))
...
df_cc %>%
  ...
  ggplot(aes(x=1, y=pct)) +
  geom_col(color="black", lwd=2, alpha=0.7, fill="#332288") +
  geom_text(aes(label=(lab)), size=7, position=position_stack(vjust=0.5), ) +
  scale_x_continuous(breaks=NULL) +
  scale_y_continuous(breaks=c(0, 50, 100), limits = c(0, 100)) +
  labs(x="", y="% Pairs Correlated") +
  ggsave(file.path(fig_dir, "SRSR_CCLPercent.svg"))
...
df_cc %>%
  ...
  scale_y_continuous(breaks=c(0, 50, 100), limits = c(0, 100)) +
  labs(x="", y="% Pairs Correlated") +
  ggsave(file.path(fig_dir, "SRSR_CCSPercent.svg"))
...
df_ev %>%
  ...
  ggplot(aes(x=1, y=pct)) +
  geom_col(color="black", lwd=2, alpha=0.7, fill="#332288") +
  # geom_text(aes(label=(lab)), size=7,) +
  scale_x_continuous(breaks=NULL) +
  scale_y_continuous(breaks=c(0, 50, 100), limits = c(0, 100)) +
  labs(x="", y="% Pairs Correlated") +
  ggsave(file.path(fig_dir, "SRSR_EvPercent.svg"))
